...
d = '/home/kenneth/proj/proXtal/proteins/fe'

outFile = open('iron_mins.txt','w')

ligHeaders = ["PDB_ID","Chain_ID","Ligand_ID","Ligand_Image","Ligand_Formula","Ligand_MW","Ligand_Name","Ligand_SMILES","InChI","InChI_Key"]

ligFile = os.path.join(d,'iron_proteins_ligands_112619.csv')
...
with open("dataOut.txt","w") as outFile:
    ...
    for row in feLig.itertuples():
            ...
            for lig in ligands:
                ...
                for atom in lig.get_atoms():
                    ...
                    if "FE" in atom.get_name():
                        ...
                        feAtom = {"PDB_ID":row.PDB_ID,"Chain_ID":row.Chain_ID,"ligName":lig.get_resname(),"ligAtom":atom.get_name(),"ligNum":atom.get_parent().get_id()[1],"ligCoords":atom.get_coord()}   
                        ...
                        for neigh in neighbors:
                            ...
                            if distance != 0:
                                neigh_dict = {"feAtom":feAtom,"Neighbors":{"Neighbor_"+str(idx): {"resName":neigh.get_parent().get_resname(),"atomName":neigh.get_name(),"resNum":neigh.get_parent().get_id()[1],"atomCoord":neigh.get_coord(),"distance":distance} for idx, neigh in enumerate(neighbors)}}
                                # print(neigh_dict['feAtom'].keys())
                                for feKey,feVal,nKey,nVal in zip(neigh_dict['feAtom'].keys(),neigh_dict['feAtom'].values(),neigh_dict["Neighbors"].keys(),neigh_dict["Neighbors"].values()):
        ...
        # print(STR.get_atoms())
        #get center iron coordinate of atoms

        # print(row.PDB_ID,row.Chain_ID,lig)
...
# d = '/home/kenneth/proj/proXtal/proteins/fe2'
# fePDBFiles = glob.glob(os.path.join(d,"*.pdb"))
...
# for f in fePDBFiles:
    ...
    # STR = PDB.PDBParser(QUIET=True).get_structure("gr",minPath)

    # for chain in STR[0]:
    #     print(chain.get_residues())

    # print(STR.get_chains())

    #check chains

    # for ch in STR.get_chains():

        

    # print([atom for atom in grPDB.get_atoms()])
    # gr_atom_list = PDB.Selection.unfold_entities(STRPRRR
    # print(gr_atom_list)
...
# for f in fePDBFiles:
    ...
    # if os.path.splitext(f)[1] == '.pdb':
        ...
        # gr_Fe = [atom for atom in grPDB.get_atoms() if "FE" in atom.get_name().upper()]
        # gr_ns = PDB.NeighborSearch(gr_atom_list)
        # gr_feNeighbors = [gr_ns.search(atom.get_coord(),5,level='A') for atom ingrace gr_Fe]
        # # print(gr_feNeighbors[0])
        # # print(gr_feNeighbors[1])
