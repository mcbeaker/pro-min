...
def get_atomDistances(structure,metalName):
    ...
    return distance12
#
def get_valFileParms(metElem,ligElem):
    # need metal elements
...
def remove_valDuplicates(mElem,lElem,dfParms):
...
def get_valence_parms(dfParms,minLigDist):
	metVal = {}#dictionary containing all val parms
	oxInt = set() #keep track of possible oxidative states that are shared between all ligands and metal
	count = 0
	...
	for key,metElem,ligElem,dist in minLigDist:
		...
		metVal[key]['lElem'] = []
		metVal[key]['Ox'] = []
		metVal[key]['bond-valence'] = []
		metVal[key]['r0'] = []
		metVal[key]['oxNum'] = 0 #ideal valence - S
		...
		for	i,oxParm in dfOxParm.iterrows():
			...
			metVal[key]['mElem'].append(oxParm.valence_param_atom_1)
			metVal[key]['lElem'].append(oxParm.valence_param_atom_2)
			metVal[key]['r0'].append(r0)
			metVal[key]['Ox'].append(Ox)
			metVal[key]['bond-valence'].append(calc_valence(r0,dist,Ox))
...
def find_feN_indX(keys):
	feNindx = [] 
	for i,key in enumerate(keys):
		if (key.find('FE') > -1) and (key.find('N') > -1):
			feNindx.append(i)
	return feNindx

def compare_valence_ox(oxInt,metVal):
	
	dif = 99
	valency = 0.0
	oxNum = 0
	
	for ox in oxInt: #loop through oxNum first for all atom_pairs
		val = 0
		oxInd = 0
		for i,key in enumerate(metVal.keys()):

			oxInd = metVal[key]['Ox'].index(ox) #assumes only one
			val += float(metVal[key]['bond-valence'][oxInd])
...
def get_valence(dists):  # inpu
	...
	dfParms = remove_valDuplicates(mElem,lElem,get_valFileParms(mElem,lElem))
	# print(dfParms)

	metVal, oxInt = get_valence_parms(dfParms,minLigDist) 
	
	dif = 99 #how close
	valence = 0.0
	
	feInd = find_feN_indX(mElem,lElem,metVal.keys())

	fe2N = pd.DataFrame()
	fe3N = pd.DataFrame()

	feKeys = find_feN_indX(metVal.keys()) #get index for feN pairs - empty if none

		# fe2N = dfParms[(dfParms.valence_param_atom_1 == 'Fe') & (dfParms.valence_param_atom_2 == 'N') & (dfParms.valence_param_atom_1_valence == 2)] 
		# fe3N = dfParms[(dfParms.valence_param_atom_1 == 'Fe') & (dfParms.valence_param_atom_2 == 'N') & (dfParms.valence_param_atom_1_valence == 3)] 
