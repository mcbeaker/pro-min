...
dfMerge = pd.read_csv(os.path.join(rdir,'nr_40','nr_40_pro_min.csv'),header=0,index_col=0)
dfMerge['findgeo'] = 'yes'
...
# print(len(dfAll[dfAll.type == 'Mineral']))
# print(len(dfAll[dfAll.type == 'Protein']))
# print(dfAll[dfAll.pdb == '1gn8'])
# #get 2018 microenv
...
# dfHag[['pdb','resName','chain','resNum','function']] = dfHag.Microenvironment_ID.str.split('[._]',expand=True)
# dfHag['resNum'] = dfHag.resNum.astype(int)
# dfHag['Binding_motif'] = dfHag.Binding_motif.astype(int)
# dfHag['hagai'] = 'yes'


#map EC to PDBs
# pdb_ec = '/home/kenneth/proj/proMin/proteins/hagai/data/pdb_chain_enzyme.csv'
# dfPDB_EC = pd.read_csv(pdb_ec)

# dfH_EC = dfMerge = pd.merge(dfHag,dfPDB_EC,how='left',left_on=['pdb','chain'],right_on=['PDB','CHAIN'])
# dfH_EC[['EC1','EC2','EC3','EC4']] = dfH_EC.EC_NUMBER.str.split('\.',expand=True)
# dfH_EC.to_csv(os.path.join(hdir,'hagai_ec.csv'))
# print(dfH_EC)
...
# print(len(dfHag.pdb.unique()))
# print(dfHag[dfHag.pdb == '1gn8'])
# print(dfHag.iloc[0])
# print(dfHag.columns)
# print(dfHag.dtypes)
# print(df.columns)

# dfMerge = pd.merge(dfAll,dfHag,how='outer',left_on=['pdb','chain','resNum'],right_on=['pdb','chain','resNum'])
...
grGroup = gr.groupby(['envComp','geo']).agg({"gRMSD":lambda x: x.std(),"valence": lambda x: x.std(),'type':lambda x:",".join(np.unique(x)),'pdb':lambda x:",".join(np.unique(x)),'Binding_motif':lambda x:list(map(int,np.unique(x.dropna())))}).reset_index()
# print(grGroup)
